...
require "../../examples/pipe"

private def capture_pipe_output : Bytes
  output = IO::Memory.new
  program = Bubbletea.new_program(
    PipeModel.new_model("artichoke"),
...
describe "examples/pipe parity" do
  it "matches the saved Go golden output exactly" do
    actual = capture_pipe_output
    expected = File.read("#{__DIR__}/golden/pipe.go.golden").to_slice
