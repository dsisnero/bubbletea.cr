# Bubble Tea v2 Upgrade Guide (Crystal Port)

This guide covers everything you need to change when upgrading from Bubble Tea
v1 to v2 in the Crystal port. For a tour of all the exciting new features, check out the
[What's New](https://github.com/charmbracelet/bubbletea/releases/tag/v2.0.0)
doc from the upstream Go repository.

> [!NOTE]
> This Crystal port follows the upstream Go API changes exactly. We don't take API changes lightly and strive to make the upgrade process as simple as possible.

## Migration Checklist

Here's the short version — a checklist you can follow top to bottom. Each item
links to the relevant section below.
...
For the Crystal port, the import paths remain the same since we maintain the same module structure.

```crystal
# Before and After (Crystal port)
require "bubbletea"
require "lipgloss"
```
...
The single biggest change in v2 is the shift from **imperative commands** to
**declarative View fields**. In v1, you'd use program options like
`tea.WithAltScreen()` and commands like `tea.EnterAltScreen` to toggle terminal
features on and off. In v2, you just set fields on the `tea.View` struct in your
`View()` method and Bubble Tea handles the rest.

This means: no more startup option flags, no more toggle commands, no more
fighting over state. Just declare what you want and Bubble Tea will make it so.

```crystal
# v1: imperative — scattered across NewProgram, Init, and Update
p = Tea::Program.new(model, Tea::Options.new(alt_screen: true, mouse_cell_motion: true))

# v2: declarative — everything lives in View()
def view : Tea::View
  v = Tea::View.new("Hello!")
  v.alt_screen = true
...
Keep this in mind as you go through the rest of the guide — most of the
"removed" things simply moved into View fields.

## View Returns a `tea.View` Now

The `View()` method no longer returns a `string`. It returns a `tea.View`
struct.
...
The `tea.View` struct has fields for everything that used to be controlled by
...
In v1, `tea.KeyMsg` was a struct you'd match on for key presses. In v2, it's an
**interface** that covers both key presses and releases. For most code, you want
`tea.KeyPressMsg`:

```crystal
# Before (v1-style mouse handling - if implemented):
case msg
when Tea::MouseMsg
  mouse = msg.mouse
...
# After (v2-style mouse handling):
case msg
when Tea::MouseClickMsg
  mouse = msg.mouse
  if mouse.button == Tea::MouseLeft
    @count += 1
  end
...
If you want to handle both presses and releases, use `tea.KeyMsg` and
type-switch inside:
...
`key.Code` is still `' '` and `key.Text` is still `" "`, but `String()` returns
`"space"`.
...
- **`key.ShiftedCode`** — the shifted key code (e.g., `'B'` when pressing
  shift+b)
- **`key.BaseCode`** — the key on a US PC-101 layout (handy for international
  keyboards)
- **`key.IsRepeat`** — whether the key is auto-repeating (Kitty protocol /
  Windows Console only)
- **`key.Keystroke()`** — like `String()` but always includes modifier info

## Paste Messages

Paste events no longer come in as `tea.KeyMsg` with a `Paste` flag. They're now
...
In v1, `tea.MouseMsg` was a struct with `X`, `Y`, `Button`, etc. In v2, it's an
**interface**. You get the coordinates by calling `msg.Mouse()`:
...
The `MouseEvent` struct is gone. The new `Mouse` struct has `X`, `Y`, `Button`,
and `Mod` fields.
...
Here's a minimal but complete program showing the most common migration patterns
side by side.

**v1 (Crystal - hypothetical v1 API):**
...
class Model
  ...
  def update(msg : Tea::Msg)
    ...
    when Tea::MouseMsg
      mouse = msg.mouse
      if mouse.button == Ultraviolet::MouseButton::Left
        @count += 1
      end
    end
    {self, nil}
...
model = Model.new
# In v1, options would be passed to Program.new
# program = Tea::Program.new(model, alt_screen: true, mouse_cell_motion: true)
program = Tea::Program.new(model)

if _model, error = program.run
  STDERR.puts error if error
  exit 1 if error
end
```

**v2 (Crystal - actual v2 API):**
...
Notice how the `NewProgram` call got simpler? All the terminal feature flags
moved into `View()` where they belong.

## Quick Reference

A flat old → new lookup table. Handy for search-and-replace and LLM-assisted
migration.
