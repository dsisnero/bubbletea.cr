...
Not needed. Covered by ultraviolet shard.
...
### Ultraviolet Equivalent Functionality

- `Ultraviolet::Terminal#make_raw` - Raw mode via termios/cfmakeraw
  (terminal_unix.cr:30)
- `Ultraviolet::Terminal#restore_tty` - Restore via tcsetattr
  (terminal_unix.cr:69)
- `Ultraviolet::Terminal#platform_size` - Size via ioctl(TIOCGWINSZ)
  (terminal_unix.cr:92)
...
Not needed. Not used by bubbletea.

The coninput package is a Windows console input library. Bubbletea doesn't
import it directly - it was likely a transitive dependency. Ultraviolet already
handles Windows console input through its own bindings.

### Ultraviolet Coverage

- `Ultraviolet::TerminalReader` for Windows (terminal_reader_windows.cr)
- Direct Windows Console API bindings (console input events, key events, mouse
  events)
...
The go-localereader package handles Windows console locale issues. Bubbletea
doesn't import it directly. Ultraviolet handles Windows console reading
natively.
...
Resolved. Use Crystal's `LibC` and `Signal` modules.
...
- Ultraviolet provides Windows Console API bindings in
  terminal_reader_windows.cr
...
2. **Crystal standard library** - LibC bindings, Signal module,
   IO::FileDescriptor
